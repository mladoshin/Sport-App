...
function TodoCalendar(props) {
  ...
  function handleCellMount(e){
    ...
    //console.log(e.el.children[0])
    //injectProgressBar(id)
    /*content.forEach((item, index) => {
      console.log(item.start)
      if(item.start == cellDateStr){
  ...
  function injectContent(args){
    ...
    const dateStr = args.date.toISOString().replace(/T.*$/, '')
    //console.log(dateStr)
  ...
  function handleAddHabit(){
    ...
    if(habit.type && habit.name && habit.startDate){
      firebase.addHabit(userId, habit)
    }
